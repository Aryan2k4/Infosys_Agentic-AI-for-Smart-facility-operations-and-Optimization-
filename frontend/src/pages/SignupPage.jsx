...
export default function SignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      // register() auto-logs-in on success (see AuthContext) — the new
      // account lands straight in the app, no separate login step needed.
      await register(username, password)
      show(`Account created — welcome, ${username}.`, 'success')
      navigate('/executive', { replace: true })
    } catch (err) {
      const detail = err.response?.data?.detail || 'Signup failed — please try a different username.'
      setError(detail)
      show(detail, 'error')
    } finally {
      setLoading(false)
    ...
    <div className="min-h-screen bg-[#0d0f13] flex items-center justify-center px-4">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="font-mono text-[9px] tracking-[0.18em] text-slate-500 mt-1">CREATE ACCOUNT</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div>
            <label className="font-mono text-[10px] tracking-widest text-slate-500 mb-1.5 block">CONFIRM PASSWORD</label>
            <div className="relative">
              <Lock size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
              <input
                type="password"
        ...
        <p className="text-center font-body text-[11px] text-slate-600 mt-5">
          Already have an account? <Link to="/login" className="text-teal-400 hover:text-teal-300 underline underline-offset-2">Log in</Link>
        </p>
        <p className="text-center font-body text-[10px] text-slate-700 mt-2 leading-relaxed">
          New accounts get technician-level access (manage records, run investigations) — not admin.
        </p>
      </motion.div>
    </div>
  )
